...
/* =========================
   ฟังก์ชันอัปโหลด avatar -> /lafs/uploads/avatars
   คืนค่า public URL (เช่น /lafs/uploads/avatars/xxxx.webp)
   ========================= */
function upload_avatar_to_public_url(string $field = 'avatar'): ?string {
  ...
  return '/lafs/uploads/avatars/' . $basename;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($pass1 !== '' || $pass2 !== '') {
    if (!hash_equals($pass1, $pass2)) {
      ...
      redirect('/lafs/admin_user_edit.php?id='.$user['id']);
    }
    if (strlen($pass1) < 8) {
...
$avatar_fallback = function_exists('avatar_fallback') ? avatar_fallback() : '/lafs/assets/img/account_circle.png';
...
<section class="section">
  <div class="form" style="max-width:560px;margin:0 auto;">
    ...
    <form method="post" action="/lafs/admin_user_edit.php?id=<?= (int)$user['id'] ?>" autocomplete="off" enctype="multipart/form-data">
      ...
      <div class="actions" style="margin-top:10px;">
        ...
        <?php if ($isRoot): ?>
          <a class="btn btn-secondary" href="/lafs/admin_users.php">ยกเลิก</a>
        <?php else: ?>
          <a class="btn btn-secondary" href="/lafs/admin_posts.php">ยกเลิก</a>
        <?php endif; ?>
      </div>
    </form>
  </div>
