...
$avatar_fallback = function_exists('avatar_fallback')
  ? avatar_fallback()
  : '/lafs/assets/img/account_circle.png';
?>
<section class="section">
  <div class="form" style="display:flex;align-items:center;justify-content:space-between;">
    ...
    <div class="actions">
      <a class="btn btn-secondary" href="/lafs/admin_posts.php">← กลับจัดการโพสต์</a>
      ...
      <article class="card" style="overflow:visible;">
        <div class="body">
            ...
            <details class="account-menu" style="position:relative;">
              ...
              <div class="dropdown" style="position:absolute;right:0;top:36px;min-width:220px;background:var(--card);border:1px solid var(--border);border-radius:12px;box-shadow:var(--shadow);padding:8px;z-index:30;">
                ...
                <div class="menu">
                  <?php if (!$isRootUser): ?>
                    <?php if ($u['is_active']): ?>
                      <form method="post" action="/lafs/admin_users.php" class="inline-form" onsubmit="return confirm('ปิดใช้งานผู้ใช้นี้?');">
                    ...
                    <?php else: ?>
                      <form method="post" action="/lafs/admin_users.php" class="inline-form">
                        <input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>">
                        <input type="hidden" name="id" value="<?= (int)$u['id'] ?>">
                    ...
                    <form method="post" action="/lafs/admin_users.php" class="inline-form" onsubmit="return confirm('รีเซ็ตรหัสผ่านและแสดงรหัสใหม่?');">
                      <input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>">
                      <input type="hidden" name="id" value="<?= (int)$u['id'] ?>">
                    ...
                    <form method="post" action="/lafs/admin_users.php" class="inline-form" onsubmit="return confirm('ลบผู้ใช้นี้ถาวร?');">
                      <input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>">
                      <input type="hidden" name="id" value="<?= (int)$u['id'] ?>">
          ...
          <div class="actions" style="margin-top:12px;">
            <a class="btn btn-secondary" href="/lafs/admin_user_edit.php?id=<?= (int)$u['id'] ?>">แก้ไข</a>
          </div>
        </div>
      </article>
    <?php endforeach; ?>
